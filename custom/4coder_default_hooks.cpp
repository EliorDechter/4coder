...
CUSTOM_DOC("Default command for responding to a startup event")
{
    ...
    if (match_core_code(&input, CoreCode_Startup)){
        ...
        u32 flags = Access_Always;
        String_Const_u8 buffer_name = SCu8("*special*");
        Buffer_Identifier bottom;
        bottom.name = "*scratch*";
        bottom.name_len = cstring_length(bottom.name);
        //TODO(wormboy): how do I get rid of the buffer has been modified message when exiting?
        bottom.id = create_buffer(app, buffer_name, BufferCreate_AlwaysNew | BufferCreate_NeverAttachToFile);
        default_4coder_panels(app, left, right, bottom);
...
default_render_buffer(Application_Links *app, View_ID view_id, Face_ID face_id,
    ...
    //CUSTOM(wormboy):
    if (g_debugger_info.is_running) {
        debugger_draw_breakpoint_annotation(app, buffer, text_layout_id, face_id);
        debugger_draw_current_line_annotation(app, buffer, text_layout_id, face_id);
    }
    
    //F4_RenderBraceLines(app, buffer, view_id, text_layout_id, face_id, cursor_pos);
...
default_render_caller(Application_Links *app, Frame_Info frame_info, View_ID view_id){
    ...
    //CUSTOM: I moved the text_layout_id to a global
    text_layout_id = text_layout_create(app, buffer, region, buffer_point);
