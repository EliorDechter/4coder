...
CUSTOM_DOC("change to special buffer") {
    //CUSTOM
    Buffer_ID buffer = get_buffer_by_name(app, SCu8((u8*)"*special*"), Access_Always);
...
function void
default_4coder_panels(Application_Links *app,
                      Buffer_Identifier left, Buffer_Identifier right, Buffer_Identifier bottom) {
    //CUSTOM(wormboy)
    Buffer_ID left_id = buffer_identifier_to_id(app, left);
    Buffer_ID right_id = buffer_identifier_to_id(app, right);
    Buffer_ID bottom_id = buffer_identifier_to_id(app, bottom);
    ...
    View_ID bottom_view = get_active_view(app, Access_Always);
    view_set_buffer(app, bottom_view, bottom_id, 0);
    view_set_split_proportion(app, bottom_view, 0.2);
    ...
    g_view_info.left_view = left_view;
    g_view_info.right_view = right_view;
    g_view_info.bottom_view = bottom_view;
}
